...
def isFilteredPath(path):
  ...
  if re.match('\/proc\/', path) is None \
    ...
    and re.match('\/home\/.*\/myapps\/perm\/', path) is None \
    and re.match('\/home\/.*\/permdir\/', path) is None \
    and re.match('\/opt\/google\/', path) is None \
...
def printGraph(pidqueue, f1, f2):
  ...
  for (k, v) in db.RangeIter(key_from='prv.db.'+pidkey+'.insertid.rowid.', key_to='prv.db.'+pidkey+'.insertid.rowid.zzz'):
    try:
      # prv.db.$pidkey.queryid.rowid.$queryid.$rowid -> $tuple
      (queryid, rowid) = k.split('.')[-2:]
      printTupleNodeEdge(queryid, rowid, v, f1)
